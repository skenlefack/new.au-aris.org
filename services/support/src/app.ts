...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  await app.register(fastifyKafka, {
    clientId: process.env['KAFKA_CLIENT_ID'] ?? 'aris-support-service',
  ...
  // --- Services ---
  const supportService = new SupportService(app.prisma, app.redis, app.kafka.producer);
  app.decorate('supportService', supportService);
