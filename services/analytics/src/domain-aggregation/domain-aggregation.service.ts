...
export class DomainAggregationService {
  private readonly logger = new Logger(DomainAggregationService.name);
  ...
  async handleLivestockCensusCreated(payload: LivestockCensusPayload): Promise<void> {
    ...
    this.logger.debug(
      `Livestock census: ${payload.countryCode} species=${payload.speciesId} pop=${payload.population}`,
    );
  }
  ...
  async handleFishCaptureRecorded(payload: FishCapturePayload): Promise<void> {
    ...
    this.logger.debug(
      `Fish capture: ${payload.countryCode} ${payload.quantityKg}kg via ${payload.gearType}`,
    );
  }
  ...
  async handleWildlifeCrimeReported(payload: WildlifeCrimePayload): Promise<void> {
    ...
    this.logger.debug(
      `Wildlife crime: ${payload.countryCode} type=${payload.crimeType}`,
    );
  }
  ...
  async handleTradeFlowCreated(payload: TradeFlowPayload): Promise<void> {
    ...
    this.logger.debug(
      `Trade flow: ${payload.countryCode} ${payload.flowDirection} $${value}`,
    );
  }
  ...
  async handleClimateHotspotDetected(payload: ClimateHotspotPayload): Promise<void> {
    ...
    this.logger.debug(
      `Climate hotspot: ${payload.countryCode} type=${payload.type} severity=${payload.severity}`,
    );
  }
  ...
  async handleApicultureProductionRecorded(payload: ApicultureProductionPayload): Promise<void> {
    ...
    this.logger.debug(
      `Apiculture production: ${payload.countryCode} ${payload.quantityKg}kg ${payload.quality}`,
    );
  }
  ...
  async handleGovernancePvsEvaluated(payload: GovernancePvsPayload): Promise<void> {
    ...
    this.logger.debug(
      `PVS evaluation: ${payload.countryCode} score=${payload.overallScore} type=${payload.evaluationType}`,
    );
  }
}
