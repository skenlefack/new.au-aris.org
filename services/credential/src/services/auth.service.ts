  ...
  async login(
    ...
    return {
      data: {
        ...
        expiresIn: 900,
        user: { id: user.id, email: user.email, firstName: user.firstName, lastName: user.lastName, role: user.role, roles: effectiveRoles, tenantId: user.tenantId, tenantLevel: user.tenant.level, domains: userDomainsResult.data, mustChangePassword: user.mustChangePassword ?? false } as any,
        permissions,
      },
    };
  }
