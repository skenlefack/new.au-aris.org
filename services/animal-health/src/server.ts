...
const SERVICE_NAME = 'animal-health';

async function main(): Promise<void> {
  const app = await buildApp();
  const port = parseInt(process.env['ANIMAL_HEALTH_PORT'] ?? '3020', 10);

  try {
    await app.listen({ port, host: '0.0.0.0' });
    app.log.info(`Animal Health service running on port ${port}`);
  } catch (err: any) {
    if (err?.code === 'EADDRINUSE') {
      console.error(
        ...
        `  3. Or change port:    ANIMAL_HEALTH_PORT=<other> in .env\n`,
      );
      process.exit(1);
    }
    throw err;
...
main().catch((err) => {
  console.error('Failed to start animal-health service:', err);
  process.exit(1);
});
