...
export async function registerProductionRoutes(app: FastifyInstance): Promise<void> {
  const authAndTenant = [app.authHookFn, tenantHook(), domainsHook('apiculture')];

  // POST /api/v1/apiculture/production — record honey production
  app.post<{ Body: CreateProductionInput }>('/api/v1/apiculture/production', {
  ...
  // GET /api/v1/apiculture/production — list production records
  app.get<{ Querystring: ProductionFilterInput }>('/api/v1/apiculture/production', {
  ...
  // GET /api/v1/apiculture/production/:id — get production record by id
  app.get<{ Params: UuidParamInput }>('/api/v1/apiculture/production/:id', {
  ...
  // PATCH /api/v1/apiculture/production/:id — update production record
  app.patch<{ Params: UuidParamInput; Body: UpdateProductionInput }>('/api/v1/apiculture/production/:id', {
