...
const SERVICE_NAME = 'knowledge-hub';

async function main(): Promise<void> {
  const app = await buildApp();
  const port = parseInt(process.env['KNOWLEDGE_HUB_PORT'] ?? '3033', 10);

  try {
    await app.listen({ port, host: '0.0.0.0' });
    app.log.info(`Knowledge Hub service running on port ${port}`);
  } catch (err: any) {
    if (err?.code === 'EADDRINUSE') {
      console.error(
        ...
        `  3. Or change port:    KNOWLEDGE_HUB_PORT=<other> in .env\n`,
      );
      process.exit(1);
    }
    throw err;
...
main().catch((err) => {
  console.error('Failed to start knowledge-hub service:', err);
  process.exit(1);
});
