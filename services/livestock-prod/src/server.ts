...
const SERVICE_NAME = 'livestock-prod';

async function main(): Promise<void> {
  const app = await buildApp();
  const port = parseInt(process.env['LIVESTOCK_PROD_PORT'] ?? '3021', 10);

  try {
    await app.listen({ port, host: '0.0.0.0' });
    app.log.info(`Livestock Production service running on port ${port}`);
  } catch (err: any) {
    if (err?.code === 'EADDRINUSE') {
      console.error(
        ...
        `  3. Or change port:    LIVESTOCK_PROD_PORT=<other> in .env\n`,
      );
      process.exit(1);
    }
    throw err;
...
main().catch((err) => {
  console.error('Failed to start livestock-prod service:', err);
  process.exit(1);
});
