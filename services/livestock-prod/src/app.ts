...
import { SlaughterService } from './services/slaughter.service.js';
import { TranshumanceService } from './services/transhumance.service.js';
import { registerCensusRoutes } from './routes/census.routes.js';
import { registerProductionRoutes } from './routes/production.routes.js';
import { registerSlaughterRoutes } from './routes/slaughter.routes.js';
import { registerTranshumanceRoutes } from './routes/transhumance.routes.js';
...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  // --- Services ---
  const censusService = new CensusService(prisma, kafka);
  const productionService = new ProductionService(prisma, kafka);
  const slaughterService = new SlaughterService(prisma, kafka);
  const transhumanceService = new TranshumanceService(prisma, kafka);

  app.decorate('censusService', censusService);
  app.decorate('productionService', productionService);
  app.decorate('slaughterService', slaughterService);
  app.decorate('transhumanceService', transhumanceService);

  // --- Health check ---
  app.get('/health', async () => ({
    status: 'ok',
    service: 'livestock-prod',
    timestamp: new Date().toISOString(),
  }));

  // --- Routes ---
  await app.register(registerCensusRoutes);
  await app.register(registerProductionRoutes);
  await app.register(registerSlaughterRoutes);
  await app.register(registerTranshumanceRoutes);
