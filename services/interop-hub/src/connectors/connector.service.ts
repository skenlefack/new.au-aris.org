...
export class ConnectorService {
  ...
  async listConnectors(): Promise<ApiResponse<ConnectorConfigEntity[]>> {
    const rows = await this.prisma.connectorConfig.findMany({
  ...
  async healthCheck(): Promise<ApiResponse<ConnectorHealth[]>> {
    ...
    this.logger.log(
      `Health check completed: ${results.length} connectors checked`,
    );
    return { data: results };
  }
   ...
   */
  async checkEndpoint(baseUrl: string): Promise<string> {
    // Adapter pattern: replace with real HTTP check in production
    try {
      this.logger.debug(`Checking endpoint: ${baseUrl}`);
