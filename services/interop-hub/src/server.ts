...
initTracing('interop-hub', '0.1.0');

import { buildApp } from './app.js';

const SERVICE_NAME = 'interop-hub';

async function main(): Promise<void> {
  const app = await buildApp();
  const port = parseInt(process.env['INTEROP_HUB_PORT'] ?? '3032', 10);

  try {
    await app.listen({ port, host: '0.0.0.0' });
    app.log.info(`Interop Hub service running on port ${port}`);
  } catch (err: any) {
    if (err?.code === 'EADDRINUSE') {
      console.error(
        ...
        `  3. Or change port:    INTEROP_HUB_PORT=<other> in .env\n`,
      );
      process.exit(1);
    }
    throw err;
...
main().catch((err) => {
  console.error('Failed to start interop-hub service:', err);
  process.exit(1);
});
