...
const SERVICE_NAME = 'data-sharing';

async function main(): Promise<void> {
  const app = await buildApp();
  const port = parseInt(process.env['DATA_SHARING_PORT'] ?? '3034', 10);

  try {
    await app.listen({ port, host: '0.0.0.0' });
    app.log.info(`Data Sharing service running on port ${port}`);
  } catch (err: any) {
    if (err?.code === 'EADDRINUSE') {
      console.error(
        `\n[${SERVICE_NAME}] Port ${port} is already in use.\n` +
          ...
          `  3. Or change port:    DATA_SHARING_PORT=<other> in .env\n`,
      );
      process.exit(1);
    }
    throw err;
...
main().catch((err) => {
  console.error('Failed to start data-sharing service:', err);
  process.exit(1);
});
