...
const SERVICE_NAME = 'fisheries';

async function main(): Promise<void> {
  const app = await buildApp();
  const port = parseInt(process.env['FISHERIES_PORT'] ?? '3022', 10);

  try {
    await app.listen({ port, host: '0.0.0.0' });
    app.log.info(`Fisheries service running on port ${port}`);
  } catch (err: any) {
    if (err?.code === 'EADDRINUSE') {
      console.error(
        ...
        `  3. Or change port:    FISHERIES_PORT=<other> in .env\n`,
      );
      process.exit(1);
    }
    throw err;
...
main().catch((err) => {
  console.error('Failed to start fisheries service:', err);
  process.exit(1);
});
