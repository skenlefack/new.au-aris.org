...
export class DashboardService {
  ...
  async getKpis(
    ...
    const [
      ...
      this.prisma.qualityReport.count({ where: { ...where, overallStatus: 'PASSED' } }),
      this.prisma.qualityReport.count({ where: { ...where, overallStatus: 'FAILED' } }),
      this.prisma.qualityReport.count({ where: { ...where, overallStatus: 'WARNING' } }),
      this.prisma.qualityReport.aggregate({ where, _avg: { totalDurationMs: true } }),
      this.prisma.correctionTracker.count({
      ...
      this.prisma.qualityViolation.groupBy({
        by: ['gate', 'severity'],
        _count: { id: true },
      }),
    ]);
    ...
    const domainStatusCounts = await Promise.all(
      domainBreakdown.map(async (d) => {
        const domainWhere = { ...where, domain: d.domain };
        const [p, f, w] = await Promise.all([
          this.prisma.qualityReport.count({ where: { ...domainWhere, overallStatus: 'PASSED' } }),
          this.prisma.qualityReport.count({ where: { ...domainWhere, overallStatus: 'FAILED' } }),
          this.prisma.qualityReport.count({ where: { ...domainWhere, overallStatus: 'WARNING' } }),
        ]);
        const t = p + f + w;
        return {
          domain: d.domain,
  ...
  private async computeAvgCorrectionTime(
    ...
    const corrected = await this.prisma.correctionTracker.findMany({
    ...
    const totalMs = corrected.reduce((sum, c) => {
