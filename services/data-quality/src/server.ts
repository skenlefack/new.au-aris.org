...
const SERVICE_NAME = 'data-quality';

async function main(): Promise<void> {
  const app = await buildApp();
  const port = parseInt(process.env['DATA_QUALITY_PORT'] ?? '3004', 10);

  try {
    await app.listen({ port, host: '0.0.0.0' });
    app.log.info(`Data Quality service running on port ${port}`);
  } catch (err: any) {
    if (err?.code === 'EADDRINUSE') {
      console.error(
        ...
        `  3. Or change port:    DATA_QUALITY_PORT=<other> in .env\n`,
      );
      process.exit(1);
    }
    throw err;
...
main().catch((err) => {
  console.error('Failed to start data-quality service:', err);
  process.exit(1);
});
