...
import type { SubmissionPayload, SyncRequestDto } from './dto/sync-request.dto';
import type {
  SyncResponse,
  SyncRejection,
  ConflictInfo,
...
export class SyncService {
  ...
  async deltaSync(
    ...
    this.logger.log(
      `Sync completed: ${accepted.length} accepted, ${rejected.length} rejected, ${conflicts.length} conflicts (${durationMs}ms)`,
    );
  ...
  private async publishSubmittedEvent(
    ...
    try {
      ...
      this.logger.error(
        `Failed to publish submitted event for ${submissionId}`,
        error instanceof Error ? error.stack : String(error),
      );
    }
  }
