...
export class CampaignService {
  ...
  async create(
    ...
    this.logger.log(
      `Campaign created: ${campaign.name} (${campaign.id}) domain=${dto.domain}`,
    );
  ...
  async findOne(
    ...
    const [totalSubmissions, validated, rejected] = await Promise.all([
      this.prisma.submission.count({ where: { campaignId: id } }),
  ...
  async update(
    ...
    this.logger.log(`Campaign updated: ${campaign.name} (${campaign.id})`);
  ...
  private async publishEvent(
    ...
    try {
      ...
      this.logger.error(
        `Failed to publish event for campaign ${c.id}`,
