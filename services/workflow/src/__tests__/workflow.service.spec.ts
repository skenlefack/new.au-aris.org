...
import { WorkflowService, HttpError } from '../services/workflow.service';
...
describe('WorkflowService', () => {
  ...
  describe('approve', () => {
    ...
    it('should throw HttpError 404 for nonexistent instance', async () => {
      ...
      try {
        await service.approve('nonexistent', undefined, dataSteward());
      } catch (e: any) {
        expect(e.statusCode).toBe(404);
      }
    });

    it('should throw HttpError 400 when instance is REJECTED', async () => {
      prisma.workflowInstance.findUnique.mockResolvedValue(
      ...
      try {
        await service.approve('wf-1', undefined, dataSteward());
      } catch (e: any) {
        expect(e.statusCode).toBe(400);
      }
    });

    it('should throw HttpError 400 when instance is already APPROVED', async () => {
      prisma.workflowInstance.findUnique.mockResolvedValue(
