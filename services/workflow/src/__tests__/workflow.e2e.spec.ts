...
describe('Workflow Full Flow — E2E (Testcontainers)', () => {
  ...
  describe('SLA escalation', () => {
    it('overdue instance is escalated by EscalationService', async () => {
      ...
      await (prisma as any).workflowInstance.update({
        where: { id: instanceId },
      ...
      const dbRow = await (prisma as any).workflowInstance.findUnique({
        where: { id: instanceId },
        include: { transitions: { orderBy: { created_at: 'asc' } } },
      });
      ...
      const escalateTransition = dbRow!.transitions.find((t: any) => t.action === 'ESCALATE');
    ...
    it('does not escalate non-overdue instances', async () => {
      ...
      const escalationService = await createEscalationService(service);
      const before = await (prisma as any).workflowInstance.findUnique({
        where: { id: created.data.id },
      });

      await escalationService.checkOverdueInstances();

      const after = await (prisma as any).workflowInstance.findUnique({
        where: { id: created.data.id },
      });
