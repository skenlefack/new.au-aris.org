...
fun LivestockCensusScreen(
    ...
    LaunchedEffect(state.isSaved) {
        if (state.isSaved) {
            val msg = if (state.saveMode == "submit") "Census submitted for sync" else "Census draft saved"
    ...
    Scaffold(
        ...
        Column(
            ...
            // Population count
            FieldLabel(stringResource(R.string.population_count), required = true)
            OutlinedTextField(
                ...
                singleLine = true,
                isError = state.errors.containsKey("population"),
                supportingText = state.errors["population"]?.let { { Text(it, color = MaterialTheme.colorScheme.error) } },
            )

            Spacer(modifier = Modifier.height(16.dp))

            // Methodology
            FieldLabel(stringResource(R.string.methodology), required = true)
            ExposedDropdownMenuBox(
                ...
                OutlinedTextField(
                    ...
                    isError = state.errors.containsKey("methodology"),
                    supportingText = state.errors["methodology"]?.let { { Text(it, color = MaterialTheme.colorScheme.error) } },
                )
                ExposedDropdownMenu(
                    expanded = state.methodologyExpanded,
            ...
            // Census date
            FieldLabel("Census Date")
            OutlinedTextField(
                value = state.censusDate?.let { dateFormat.format(Date(it)) } ?: "",
                onValueChange = {},
                readOnly = true,
                modifier = Modifier.fillMaxWidth(),
            ...
            // Geographic unit
            FieldLabel("Location (Admin Unit)")
            ExposedDropdownMenuBox(
                expanded = state.geoExpanded,
                onExpandedChange = { viewModel.toggleGeoDropdown() },
            ) {
                OutlinedTextField(
                    value = state.selectedGeoName,
                    onValueChange = {},
                    readOnly = true,
                    modifier = Modifier
                        .fillMaxWidth()
                ...
                ExposedDropdownMenu(
                    expanded = state.geoExpanded,
                    onDismissRequest = { viewModel.toggleGeoDropdown() },
                ) {
                    geoList.forEach { geo ->
                        DropdownMenuItem(
                            text = { Text(geo.name) },
                            onClick = { viewModel.selectGeo(geo.id, geo.name) },
                        )
                    }
                }
            }
    ...
    if (showDatePicker) {
        val datePickerState = rememberDatePickerState(initialSelectedDateMillis = state.censusDate)
        DatePickerDialog(
            onDismissRequest = { showDatePicker = false },
            confirmButton = {
                TextButton(onClick = {
                    datePickerState.selectedDateMillis?.let { viewModel.setCensusDate(it) }
