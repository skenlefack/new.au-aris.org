...
function SubDomainsList() {
  ...
  const { data, isLoading, error } = useAdminSubDomains({
    page,
    limit: PAGE_SIZE,
  ...
  const filtered = useMemo(() => {
    ...
    return subDomains.filter(
      (sd) =>
        ...
        (sd.valueChainCode?.toLowerCase().includes(q) ?? false),
    );
  }, [subDomains, search]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (err?.statusCode === 409) {
        setDeleteError(err.message || 'Ce sous-domaine est utilise par des entites existantes et ne peut pas etre supprime.');
      } else {
    ...
    <div className="space-y-6 pb-20">
                      ...
                      >
                        <span
                          className={`inline-block h-4 w-4 rounded-full bg-white shadow transition-transform ${
                            sd.active ? 'translate-x-4' : 'translate-x-0'
                          }`}
                        />
                      </button>
                    </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Page {meta.page} sur {totalPages} ({meta.total} resultats)
          </p>
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={page <= 1}
              className="rounded-lg border border-gray-200 p-2 text-sm text-gray-600 transition-colors hover:bg-gray-50 disabled:opacity-40 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700"
            >
