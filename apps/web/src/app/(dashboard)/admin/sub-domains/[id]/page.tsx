...
const TYPE_OPTIONS: { value: SubDomainType; label: string }[] = [
  { value: 'VALUE_CHAIN', label: 'Chaine de valeur' },
...
function SubDomainDetail() {
  ...
  const handleToggleActive = async () => {
    ...
    try {
      ...
      addToast({
        type: 'success',
        title: sd.active ? 'Desactive' : 'Active',
  ...
  if (error || !sd) {
    return (
      <div className="mx-auto max-w-2xl py-12 text-center">
        ...
        <Link href="/admin/sub-domains" className="mt-4 inline-block text-sm text-[#1F4E79] hover:underline">
          Retour a la liste
        </Link>
      </div>
    );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              >
                {sd.active ? 'Actif' : 'Inactif'}
              </span>
            </div>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {sd.labelFr} &mdash; {parentDomain?.name?.en ?? sd.domainCode}
            </p>
          </div>
        </div>
            ...
            <>
              ...
              <button
                ...
                className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
                  sd.active
                    ? 'border border-amber-200 text-amber-700 hover:bg-amber-50 dark:border-amber-800 dark:text-amber-400 dark:hover:bg-amber-900/20'
                    : 'border border-emerald-200 text-emerald-700 hover:bg-emerald-50 dark:border-emerald-800 dark:text-emerald-400 dark:hover:bg-emerald-900/20'
                }`}
              >
      ...
      <div className="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
        {editing ? (
          ...
          <div className="space-y-5 p-6">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Modifier le sous-domaine</h3>

            {/* Info: code is read-only */}
            ...
            <div className="space-y-1.5">
              ...
              <select
                value={typeEnum}
                onChange={(e) => {
                  setTypeEnum(e.target.value as SubDomainType);
                  if (e.target.value !== 'VALUE_CHAIN') setValueChainCode('');
                }}
            ...
            <div className="space-y-1.5">
              ...
              <select
                value={valueChainCode}
                onChange={(e) => setValueChainCode(e.target.value)}
              ...
              >
                ...
                {valueChainCodes.map((vc) => (
                  <option key={vc.code} value={vc.code}>{vc.labelFr || vc.labelEn} ({vc.code})</option>
                ))}
              </select>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <input type="number" min={0} value={displayOrder} onChange={(e) => setDisplayOrder(parseInt(e.target.value, 10) || 0)}
                className="w-40 rounded-lg border border-gray-200 px-3 py-2 text-sm shadow-sm focus:border-[#1F4E79] focus:outline-none focus:ring-1 focus:ring-[#1F4E79] dark:border-gray-700 dark:bg-gray-900 dark:text-white"
              />
            </div>
  ...
  return (
    <div className="flex items-start gap-4 px-6 py-3">
      ...
      <div className="flex-1">
        {children ?? (
          <span
            className={`text-sm text-gray-900 dark:text-white ${mono ? 'rounded bg-gray-100 px-2 py-0.5 font-mono dark:bg-gray-700' : ''}`}
            dir={dir}
          >
            {value}
          </span>
