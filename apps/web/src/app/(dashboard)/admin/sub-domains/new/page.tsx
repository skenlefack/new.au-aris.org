...
function SubDomainForm() {
  ...
  const validate = (): boolean => {
    ...
    if (typeEnum === 'VALUE_CHAIN' && !valueChainCode) {
      // optional per spec, but show warning
    }
    setErrors(e);
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6 pb-20">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Creer un nouveau sous-domaine dans le systeme ARIS.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
            ...
            {TYPE_OPTIONS.map((opt) => (
              ...
              >
                <input
                  ...
                  name="typeEnum"
                  value={opt.value}
                  checked={typeEnum === opt.value}
                  onChange={() => {
                    setTypeEnum(opt.value);
                    if (opt.value !== 'VALUE_CHAIN') setValueChainCode('');
                  }}
        ...
        {/* Value Chain Code (only if type === VALUE_CHAIN) */}
        <div className="space-y-1.5">
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
            Code chaine de valeur
            {typeEnum === 'VALUE_CHAIN' && <span className="ml-1 text-xs text-gray-400">(optionnel)</span>}
          </label>
          <select
            name="valueChainCode"
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              Label AR <span className="text-xs text-gray-400">(optionnel)</span>
            </label>
            <input
              type="text"
              name="labelAr"
          ...
          <div className="space-y-1.5">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              Label PT <span className="text-xs text-gray-400">(optionnel)</span>
            </label>
            <input
              type="text"
              name="labelPt"
