...
const EXTRA_ALIASES: Record<string, string> = {
  ...
  'united republic of tanzania': 'TZ', 'central african republic': 'CF', 'car': 'CF',
...
Object.assign(COUNTRY_NAME_TO_ISO2, EXTRA_ALIASES);
  ...
  return (
    <div className="flex items-center gap-4">
      <div
        ...
        style={{ width: size, height: size, background: entries.length > 0 ? bg : '#e5e7eb' }}
      />
...
function DonutChart({
  ...
  return (
    <div className="flex items-center gap-4">
      <div
        className="relative shrink-0 rounded-full shadow-lg"
        style={{ width: size, height: size, background: entries.length > 0 ? bg : '#e5e7eb' }}
      >
        <div
          className="absolute rounded-full bg-white dark:bg-gray-900 flex items-center justify-center"
          style={{ inset: size * 0.2 }}
        >
          {centerLabel && (
            <span className="text-lg font-bold text-gray-700 dark:text-gray-200">{centerLabel}</span>
          )}
        </div>
      </div>
      <div className="space-y-2">
...
/* ================================================================== */
/*  Digital Tools Dashboard Page                                       */
/* ================================================================== */

export default function DigitalToolsDashboardPage() {
  // Fetch PPR surveillance campaign submissions
  const cQ = useCollectionCampaigns({ limit: 100 });
  const campaigns: any[] = Array.isArray(cQ.data?.data) ? cQ.data.data : [];

  // Find surveillance campaign (Campaign A)
  const survCampaign = useMemo(() =>
    campaigns.find((c: any) => {
      const name = (typeof c.name === 'object' ? (c.name?.fr || c.name?.en || '') : String(c.name || '')).toLowerCase();
      return name.includes('surveillance') && (name.includes('outil') || name.includes('numérique') || name.includes('digital'));
    }),
  [campaigns]);

  const sQ = useCampaignSubmissions(survCampaign?.id, { limit: 5000 });
  const rawSubs: any[] = Array.isArray(sQ.data?.data) ? sQ.data.data : [];

  const loading = cQ.isLoading || sQ.isLoading;

  // Process submissions
  const { countriesInfo, kpis, mapData, toolsByDeveloper, challengesList } = useMemo(() => {
    ...
    for (const sub of rawSubs) {
      ...
      byCountry.set(code, {
        code,
        name: countryGeo.nameFr || countryGeo.name,
        status: usesDigital ? 'uses_digital' : 'no_digital',
        toolName: d.tool_name || undefined,
        toolDeveloper: d.tool_developer || undefined,
        hasSurveillance: d.has_surveillance === 'yes',
    ...
    // Map data — color-coding:
    // uses_digital → high value (green on choropleth), no_digital → 0 (gray)
    ...
    for (const c of all) {
      if (c.toolDeveloper) {
        devMap.set(c.toolDeveloper, (devMap.get(c.toolDeveloper) || 0) + 1);
      }
    }
    const devArr = Array.from(devMap.entries())
      .map(([dev, count]) => ({ label: dev, value: count }))
      .sort((a, b) => b.value - a.value);

    // Challenges
    const challenges: string[] = [];
    for (const sub of rawSubs) {
      const ch = sub.data?.main_challenges;
  ...
  // Countries with digital tools (for table)
  const digitalCountries = useMemo(
    () => countriesInfo.filter((c) => c.status === 'uses_digital').sort((a, b) => a.name.localeCompare(b.name)),
    [countriesInfo],
  );
  const noDigitalCountries = useMemo(
    () => countriesInfo.filter((c) => c.status === 'no_digital').sort((a, b) => a.name.localeCompare(b.name)),
    [countriesInfo],
  );

  // All 55 AU countries with digital status overlay
  const allCountriesStatus = useMemo(() => {
    ...
    return AFRICA_COUNTRIES.map((c) => ({
      ...c,
      status: statusMap.get(c.code) ?? ('not_surveyed' as DigitalStatus),
    }));
  }, [countriesInfo]);

  const statusColors: Record<DigitalStatus, string> = {
    uses_digital: '#059669',
    no_digital: '#DC2626',
    not_surveyed: '#D1D5DB',
  };
  const statusLabels: Record<DigitalStatus, string> = {
    uses_digital: 'Utilise un outil numérique',
    no_digital: 'Pas d\'outil numérique',
    not_surveyed: 'Non enquêté',
  };

  return (
    <div className="flex min-h-[calc(100vh-56px)] flex-col bg-[#f0f2f5] dark:bg-gray-950">
      {/* ── HEADER ── */}
      ...
      <div className="grid grid-cols-5 divide-x divide-gray-200 border-b border-gray-300 bg-white dark:divide-gray-700 dark:bg-gray-900">
        {loading ? (
          ...
          [
            ...
            { icon: Smartphone, val: kpis.withSurveillance, label: 'Avec système de surveillance', color: '#EA580C', bg: '#FFF7ED' },
          ].map((k, i) => {
            const Icon = k.icon;
            return (
      ...
      {loading ? (
        ...
        <div className="flex-1 overflow-auto p-4 space-y-4">
          {/* ROW 1: Map + Status Pie */}
          <div className="grid gap-4 lg:grid-cols-5">
            {/* Africa Map — spans 3 cols */}
              ...
              <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3 dark:border-gray-800">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200">
                    Carte des Outils Numériques en Afrique
                  </h3>
                </div>
              </div>
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-900">
                ...
                <PieChart
                  entries={[
                    { label: 'Utilise un outil', value: kpis.withDigital },
                    { label: 'Sans outil', value: kpis.withoutDigital },
                  ]}
              ...
              {/* Donut: Surveillance vs No surveillance */}
              <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-900">
                <h3 className="mb-4 text-sm font-bold text-gray-800 dark:text-gray-200">
                ...
                <DonutChart
                  entries={[
                    { label: 'Avec surveillance', value: kpis.withSurveillance },
                    { label: 'Sans surveillance', value: kpis.surveyed - kpis.withSurveillance },
                  ]}
                  colors={['#1E40AF', '#F59E0B']}
                  size={140}
                  centerLabel={`${Math.round((kpis.withSurveillance / (kpis.surveyed || 1)) * 100)}%`}
                />
              </div>

              {/* Donut: Own system vs External */}
              <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-900">
                <h3 className="mb-4 text-sm font-bold text-gray-800 dark:text-gray-200">
                  Type de Système Numérique
                </h3>
                <DonutChart
                  entries={[
                    { label: 'Système propre', value: kpis.withOwnSystem },
                    { label: 'Aucun système', value: kpis.surveyed - kpis.withOwnSystem },
                  ]}
                  colors={['#7C3AED', '#9CA3AF']}
                  size={140}
                  centerLabel={`${kpis.withOwnSystem}`}
                />
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {/* Table: Countries WITH digital tools */}
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
              <div className="flex items-center gap-2 border-b border-gray-100 px-4 py-3 dark:border-gray-800">
                <Monitor className="h-4 w-4 text-green-600" />
                <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200">
                  Pays utilisant les outils numériques ({digitalCountries.length})
                </h3>
              </div>
              <div className="overflow-auto max-h-[360px]">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    {digitalCountries.length === 0 && (
                      <tr><td colSpan={3} className="px-4 py-6 text-center text-gray-400 italic">Aucune donnée</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            {/* Table: Countries WITHOUT digital tools */}
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
              <div className="flex items-center gap-2 border-b border-gray-100 px-4 py-3 dark:border-gray-800">
                <MonitorOff className="h-4 w-4 text-red-600" />
                <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200">
                  Pays sans système numérique ({noDigitalCountries.length})
                </h3>
              </div>
              <div className="overflow-auto max-h-[360px]">
                <table className="w-full text-sm">
          ...
          {/* ROW 3: Developer distribution + Non-surveyed countries */}
          <div className="grid gap-4 lg:grid-cols-2">
            {/* Pie: Tools by developer */}
            {toolsByDeveloper.length > 0 && (
              <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-900">
                <h3 className="mb-4 text-sm font-bold text-gray-800 dark:text-gray-200">
                  Outils par Développeur
                </h3>
                <PieChart
                  entries={toolsByDeveloper}
                  colors={['#1E40AF', '#059669', '#EA580C', '#7C3AED', '#DC2626', '#D97706', '#0891B2', '#4F46E5']}
                  size={140}
                />
              </div>
            )}

            {/* Non-surveyed countries grid */}
            <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-900">
              ...
              <div className="flex flex-wrap gap-1.5">
                {allCountriesStatus
                  .filter((c) => c.status === 'not_surveyed')
                  .sort((a, b) => a.nameFr.localeCompare(b.nameFr))
                  .map((c) => (
                    <span
                      key={c.code}
                      className="rounded-md border border-gray-200 bg-gray-50 px-2 py-1 text-xs text-gray-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400"
                    >
                      {c.nameFr}
                    </span>
                  ))}
