...
export default function VetEvaluationsPage() {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('vetEvaluations')}</h1><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('vetEvaluationsDesc')}</p></div>
        </div>
        <Link href={`/collecte/forms/${PRIMARY}/fill?returnTo=/governance/vet-evaluations`} className="flex items-center gap-2 rounded-lg bg-blue-700 px-3 py-2 text-sm font-semibold text-white hover:bg-blue-800"><Plus className="h-4 w-4" /> New Evaluation</Link>
      </div>
      ...
      : !filtered.length ? <div className="flex flex-col items-center py-16 text-center"><Landmark className="h-12 w-12 text-gray-200 dark:text-gray-600" /><p className="mt-4 text-sm text-gray-400">{t('noEvaluationsFound')}</p><Link href={`/collecte/forms/${PRIMARY}/fill?returnTo=/governance/vet-evaluations`} className="mt-4 flex items-center gap-1 text-sm font-medium text-blue-600 hover:text-blue-700"><Plus className="h-4 w-4" /> New Evaluation</Link></div>
