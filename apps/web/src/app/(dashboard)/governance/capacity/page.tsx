...
import { ArrowLeft, Search, Filter, Plus, FileText, Clock, CheckCircle2, XCircle, GraduationCap } from 'lucide-react';
import { cn } from '@/lib/utils';
import { Skeleton } from '@/components/ui/Skeleton';
import { useMultiTemplateSubmissions } from '@/lib/api/form-builder-hooks';
import { useTranslations } from '@/lib/i18n/translations';

const TEMPLATES = ['7efe40cd-de3c-4cfb-a950-aee83d920de9'];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('capacity')}</h1><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('capacityDesc')}</p></div>
        </div>
        <Link href={`/collecte/forms/${PRIMARY}/fill?returnTo=/governance/capacity`} className="flex items-center gap-2 rounded-lg bg-green-700 px-3 py-2 text-sm font-semibold text-white hover:bg-green-800"><Plus className="h-4 w-4" /> New Report</Link>
      </div>
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 sm:max-w-xs"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" /><input type="text" placeholder="Search capacity..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full rounded-lg border border-gray-200 py-2 pl-9 pr-3 text-sm placeholder:text-gray-400 focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-200 dark:border-gray-700 dark:bg-gray-900 dark:text-white" /></div>
        <div className="flex items-center gap-2"><Filter className="h-4 w-4 text-gray-400" /><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-700 focus:border-green-500 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300"><option value="">{t('allStatus')}</option><option value="SUBMITTED">Submitted</option><option value="VALIDATED">Validated</option><option value="REJECTED">Rejected</option><option value="DRAFT">Draft</option></select></div>
          ...
          return (
            <div key={sub.id} className="group relative overflow-hidden rounded-xl border border-gray-200 bg-white p-5 transition-all hover:-translate-y-0.5 hover:shadow-lg dark:border-gray-700 dark:bg-gray-800">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                <div><p className="text-lg font-bold text-gray-900 dark:text-white">{d.staff_count ? Number(d.staff_count).toLocaleString() : '—'}</p><p className="text-[10px] text-gray-400">Staff</p></div>
                <div><p className="text-lg font-bold text-gray-900 dark:text-white">{d.budget_usd ? `$${(Number(d.budget_usd) / 1_000_000).toFixed(1)}M` : '—'}</p><p className="text-[10px] text-gray-400">Budget</p></div>
                <div><p className="text-sm font-bold text-gray-900 dark:text-white">{d.year || '—'}</p><p className="text-[10px] text-gray-400">Year</p></div>
              </div>
              <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                {cc && <span className="rounded bg-gray-100 px-1.5 py-0.5 font-medium dark:bg-gray-700">{cc}</span>}
                {d.oie_status && <span>{d.oie_status}</span>}
              </div>
            </div>);
        })}</div>}
    </div>
