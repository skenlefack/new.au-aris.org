...
export function PAIDBenefByCountry({ agg, t }: BarProps) {
  const entries = Array.from(agg.byCountry.values())
    .sort((a, b) => b.quantityImplemented - a.quantityImplemented)
    .slice(0, 15);
  const max = entries[0]?.quantityImplemented || 1;

  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
      <h4 className="mb-4 text-sm font-semibold text-gray-900 dark:text-white">Quantity implemented by country</h4>
      <div className="space-y-2">
        {entries.map((e) => {
          ...
          return (
            <div key={e.country} className="flex items-center gap-2">
              ...
              <span className="w-16 shrink-0 text-right text-[10px] font-semibold text-gray-900 dark:text-white">
                {e.quantityImplemented >= 1_000_000 ? `${(e.quantityImplemented / 1_000_000).toFixed(1)}M` : e.quantityImplemented.toLocaleString()}
              </span>
            </div>
          );
        })}
...
export function PAIDBenefByActivity({ agg, t }: BarProps) {
  const entries = Array.from(agg.byActivity.values())
    .sort((a, b) => b.submissions - a.submissions)
    .slice(0, 8);
  const total = entries.reduce((s, e) => s + e.submissions, 0) || 1;

  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
      <h4 className="mb-4 text-sm font-semibold text-gray-900 dark:text-white">Submissions by activity</h4>
      <div className="space-y-2">
        {entries.map((entry, idx) => {
          const pct = Math.round((entry.submissions / total) * 100);
          ...
          return (
            <div key={entry.label}>
              <div className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: color }} />
                  <span className="truncate text-gray-700 dark:text-gray-300" title={entry.label}>{entry.label.length > 40 ? `${entry.label.slice(0, 40)}...` : entry.label}</span>
                </div>
...
export function PAIDActivitiesBar({ agg, t }: BarProps) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
      <h4 className="mb-4 text-sm font-semibold text-gray-900 dark:text-white">Top activities by quantity</h4>
      <div className="space-y-2">
        {entries.map((entry, idx) => {
          const pct = Math.round((entry.quantityImplemented / max) * 100);
          const color = ACTIVITY_COLORS[idx % ACTIVITY_COLORS.length];
          return (
            <div key={entry.label} className="flex items-center gap-2">
              <span className="w-48 shrink-0 truncate text-right text-[10px] font-medium text-gray-600 dark:text-gray-400" title={entry.label}>{entry.label}</span>
              <div className="h-4 flex-1 overflow-hidden rounded bg-gray-100 dark:bg-gray-700">
                <div className="h-full rounded transition-all" style={{ width: `${pct}%`, backgroundColor: color }} />
              </div>
              <span className="w-14 shrink-0 text-right text-[10px] font-semibold text-gray-900 dark:text-white">{entry.quantityImplemented.toLocaleString()}</span>
            </div>
          );
        })}
      </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-700">
              ...
              <th className="pb-2 text-right font-semibold text-gray-500">Qty Impl.</th>
            ...
            {entries.map((e) => {
              ...
              return (
                <tr key={e.sector} className="border-b border-gray-100 dark:border-gray-800">
                  ...
                  <td className="py-2 text-right font-semibold text-gray-900 dark:text-white">{e.quantityImplemented.toLocaleString()}</td>
