...
| `TOPIC_AU_QUALITY_RECORD_REJECTED` | `au.quality.record.rejected.v1` | data-quality, collecte | message, analytics | `{ reportId, recordId, entityType, overallStatus, errors[] }` |
...
| `TOPIC_AU_WORKFLOW_WAHIS_READY` | `au.workflow.wahis.ready.v1` | workflow | interop-hub, message | `{ instanceId, entityType, entityId, domain }` |
| `TOPIC_AU_WORKFLOW_ANALYTICS_READY` | `au.workflow.analytics.ready.v1` | workflow | analytics | `{ instanceId, entityType, entityId, domain }` |

## Animal Health Topics (7)
